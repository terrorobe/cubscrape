  ...
  setup() {
    ...
    const buildSQLQuery = (filterValues) => {
      ...
      if (filterValues.selectedTags && filterValues.selectedTags.length > 0) {
        ...
        const tagConditions = filterValues.selectedTags.map(
          () => 'g.tags LIKE ?',
        )
      ...
      if (filterValues.timeFilter && filterValues.timeFilter.type) {
        ...
        if (
          ...
          if (timeFilter.startDate && timeFilter.endDate) {
            query +=
              ' AND g.latest_video_date >= ? AND g.latest_video_date <= ?'
          ...
          if (timeFilter.startDate && timeFilter.endDate) {
            const startSortable = parseInt(
              timeFilter.startDate.replace(/-/g, ''),
            )
          ...
          switch (timeFilter.smartLogic) {
            ...
            case 'old-game-new-attention':
              ...
              params.push(
                parseInt(
                  oneYearAgo.toISOString().slice(0, 10).replace(/-/g, ''),
                ),
              )
              params.push(thirtyDaysAgo.toISOString())
              break
