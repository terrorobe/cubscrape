...
export default {
  ...
  setup(props, { emit }) {
    ...
    const currentSortInfo = computed(() => {
      return sortDescriptions[currentSort.value] || ''
    })

    // Contextual sorting options based on active filters
    const contextualSortOptions = computed(() => {
      const options = []
    ...
    const addSecondaryCriteria = () => {
      hasSecondaryCriteria.value = true
      handleAdvancedSortChange()
    }

    const removeSecondaryCriteria = () => {
    ...
    const initializeFromSort = (sortValue) => {
    ...
    return {
      ...
      quickPresets,
      contextualSortOptions,
      currentSortInfo,
      advancedSortInfo,
      toggleAdvancedMode,
      addSecondaryCriteria,
      removeSecondaryCriteria,
      applyPreset,
      handleSortChange,
      handleAdvancedSortChange,
    }
